...
var ShipComponent = /** @class */ (function () {
    ...
    ShipComponent.prototype.getUpgradeByName = function (upgrade) {
        if (!upgrade)
            return '';
        var upgrades = this.upgrades.filter(function (u) { return upgrade.name === u.name; });
    ...
    ShipComponent.prototype.checkUpgrade = function (upgrade) {
        ...
        var requirementSet = !isRemove && this.ship[upgradeRequirement].name === requirementName || isRemove && this.ship[upgradeRequirement].name !== requirementName;
        if (!requirementSet)
            this.ship[upgrade] = '';
    ...
    ShipComponent.prototype.updatePoints = function () {
        var _this = this;
        var cost = 0;
        this.form.valueChanges.subscribe(function (value) {
            cost = _this.ship.cost;
            Object.keys(value).forEach(function (upgrade) {
                if (_this.form.controls[upgrade].value.cost) {
                    cost = cost + parseInt(_this.form.controls[upgrade].value.cost);
                }
            });
            _this.cost = cost;
        });
...
var SquadComponent = /** @class */ (function () {
    ...
    SquadComponent.prototype.pointsForUpgrade = function (ship, upgrades, upgrade) {
        if (!upgrade)
            return 0;
        upgrades = upgrades.filter(function (u) { return upgrade.name.split(",")[0].indexOf(u.name) > -1; });
